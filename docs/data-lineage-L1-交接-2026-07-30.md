...
L1 第 1 批（T0/T1/T2）**已实现并提交**，12 个单测通过；但同批已识别出 **5 项返工（R1-R5）**，
...
|  批次   |               任务               |          状态           |
|-------|--------------------------------|-----------------------|
| 第 1 批 | T0 基准脚手架 · T1 DDL · T2 内存图+协调器 | ✅ 已提交，**但有 R1-R5 返工** |
...
| R5 | 事务边界收归 Coordinator（`TransactionTemplate`），不由 loader 自觉          | 契约只在 Javadoc 时，隔离级别配错**编译通过且测试全绿**，线上偶发撕裂快照     |
