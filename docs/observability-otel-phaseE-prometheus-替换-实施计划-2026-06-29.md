...
| 阶段 |               内容               |       验证        |   状态   |
|----|--------------------------------|-----------------|--------|
| S1 | `otelcol.ftl` 通用 scrape 渲染     | 渲染单测绿           | ✅ 已完成  |
| S2 | API 侧本地就近 scrape 生成器 + job 注册表 | 生成器单测绿          | ✅ 已完成  |
| S3 | 触发点接线（替换 PrometheusService 调用） | 启动/纳管/停服触发 push | ✅ 已完成  |
| S4 | Prometheus 全链退役 + DML 清理       | 编译无残引用          | ✅ 已完成  |
| S5 | 端到端验证                          | 真实/沙箱抓取落 Doris  | 🟡 进行中 |
...
- **S1（2026-06-30）已完成**：
  - `otelcol.ftl` 已移除写死的 `prometheus/doris` 分支，改为通过 `localScrapeJobsYaml` 注入通用
    `prometheus/local` receiver。
  - metrics pipeline 仅在 `localScrapeJobsYaml` 非空时引用 `prometheus/local`，并与 `exporterMode`
    解耦；`exporterMode=s3` 时仍可渲染本地 scrape。
  - 验证：`OtelcolTemplateTest` 覆盖非空、空值、S3 模式三类场景。
- **S2（2026-06-30）已完成**：
  - 新增 `OtelScrapeConfigBuilder`，按 `clusterId + hostname` 查询本机 RUNNING 角色，基于
    `ServiceRoleJmxMap` 生成 `127.0.0.1:<port>` scrape job。
  - 已覆盖 `PATH_OVERRIDES`、Doris FE/BE `group` label、无端口跳过、空角色仅 node_exporter 等场景。
  - 偏差记录：DolphinScheduler `ApiServer` 使用旧 `prometheus.ftl` 中的真实路径
    `/dolphinscheduler/actuator/prometheus`，不是计划表里泛化写法 `/actuator/prometheus`。
- **S3（2026-06-30）已完成**：
  - `OtelCollectorConfigService` 自动合并 OTELCOLLECTOR 服务配置、调用 `OtelScrapeConfigBuilder`
    注入 `localScrapeJobsYaml`，并保持 configure 成功后 restart。
  - `WorkerStartService`、`ServiceCommandService`、`OtelExporterSwitchService` 已移除旧
    `PrometheusService` 触发链，改为刷新受影响 hostname 的 otelcol 配置。
  - 告警启停、主机/角色删除相关路径已去除旧 Prometheus rule/config 生成逻辑。
- **S4（2026-06-30）已完成**：
  - 已删除 Prometheus meta、worker 模板、安装 handler、API `PrometheusService`、旧 command/model、
    manifest 包记录，并在 `V2.2.2__DML.sql` 追加 PROMETHEUS 告警种子数据清理。
  - 已清理 `FreemakerUtils`、worker handler SPI、API gRPC client/adapter 等业务层旧调用。
  - 保留 `PrometheusProxy*` 作为看板迁移期入口；`worker.proto`/generated stub 中的
    `GenerateAlertConfig` 仅作协议兼容残留，业务代码不再调用。
  - 验证：Prometheus 业务残引用扫描无输出；`package/manifest.json` 可被 `python3 -m json.tool` 正常解析。
- **S5（2026-06-30）部分完成，待真实/沙箱环境验证**：
  - 已通过聚焦测试：
    `./mvnw -pl datasophon-worker -am -Dtest=OtelcolTemplateTest -Dspotless.check.skip=true test`
    （7 tests passed）。
  - 已通过聚焦测试：
    `./mvnw -pl datasophon-api -am -Dtest=OtelScrapeConfigBuilderTest,OtelExporterSwitchServiceTest,OtelCollectorConfigServiceTest,WorkerCommandClientTest -Dspotless.check.skip=true test`
    （27 tests passed）。
  - 已通过编译验证：
    `./mvnw -pl datasophon-api -am compile -DskipTests -Dspotless.check.skip=true`。
  - 已执行 `./mvnw spotless:apply`，并通过 `git diff --check`。
  - 未完成：本机无可用 MySQL/Doris 沙箱，`datasophon-api` 全量测试会被 Spring 定时任务/Druid
    后台线程持续连接 `127.0.0.1:3306/datasophon` 阻塞；真实安装组件后检查 otelcol 配置和 Doris
    `otel` 表入库仍需在可用环境中执行。
