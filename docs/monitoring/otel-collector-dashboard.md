...
| 拒收 / 丢弃速率   | `otelcol_receiver_refused_*` + `otelcol_processor_dropped_*` + `otelcol_processor_filter_datapoints_filtered` | `otel_metrics_sum`   | `rate(1m)`，分别按 `receiver,transport` / `processor` 分组 | 条/s |
