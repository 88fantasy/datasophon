...
```ts
import { CHART_COLORS, colorByThreshold, formatBytes, formatCompact } from '../PrometheusMonitor/utils/formatters';
```
...
# 复用的基础组件（来自 PrometheusMonitor/panels/、utils/、service.ts）
...
datasophon-ui-v2/src/pages/DorisMonitor/
  ...
  └── (panels/ 和 utils/ 直接引用 PrometheusMonitor/ 中的共享组件，无需复制)
```
