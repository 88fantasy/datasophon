...
GET /ddh/api/v2/prometheus/query   # 用于 instant query（stat/singlestat 面板）
  ?clusterId={clusterId}
  &query={PromQL}
  &time={unix_ts}
```
...
>
> ```json
> { "success": true, "code": 200, "data": { "resultType": "matrix", "result": [...] } }
> ```
>
> 前端 service.ts 取 `res.data` 即得到 `PrometheusMatrix | PrometheusVector`。
...
datasophon-ui-v2/src/pages/ApisixMonitor/
  ├── index.tsx               # 主页面（路由 /monitor/apisix）
  ├── panels/
  │   ├── StatPanel.tsx
  │   ├── StatusStatPanel.tsx
  │   ├── TimeSeriesPanel.tsx
...
**原因**：UMI `base: '/ddh'` 让所有 React 路由都带 `/ddh` 前缀，proxy 规则会把页面导航请求（`GET /ddh/monitor/apisix`）也转发到后端，返回 504。`bypass` 让 `text/html` 请求由 dev server 自己处理（historyApiFallback → index.html）。
