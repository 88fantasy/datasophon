...
> | **2026-08-25（二次）** | **§2.2 结论加固**：对 OpenLineage 最新版复核，确认 `outputStatistics` 的缺失**与版本无关、无替代字段、社区无在办提案**，并给出根因；口径精确为「拿不到**权威**行数，但拿得到**观测**行数」。**§2.3 / §5.4 / §9.4 补入累计量的精度实测**：Delta 累加在语义上正确，但精度受采集完整性支配（已量化）。**§6.3 补入指标覆盖不一致**。**§9.3 补入 OpenLineage 版本下限** |
...
展示不难，难在归属。DAG 上的节点是调度任务，而行数和速率属于计算作业。调度系统和计算引擎各有一套身份体系：DS 认 `taskInstanceId`，Spark 认 `spark.app.id`，Flink 认 32 位 hex 的 `JobID`。把它们对上号本应由 DS 的 `app_link` 字段承担，但该字段在 DS 3.4.1 上对 Flink/Spark 任务恒为空（§2.1）。这是上游在 3.3/3.4 执行器重构中引入的回归，不是配置问题。

绕开它有四条路：解析任务日志、按作业名匹配、给上游打补丁，或者让平台自己把调度标识注入到计算作业里。本方案选最后一条，因为只有它在出错时是静默失败而不是静默错配。注入的标识要么完整传回、要么完全没有，不会像名字匹配那样把 A 作业的行数显示到 B 任务上。

另一个约束来自引擎能力。行数与容量依赖 OpenLineage 的 `outputStatistics` facet，而这个 facet 只有 Spark 集成实现，Flink 集成没有（§2.2）。所以批与流的取数路径不同：批任务的量走血缘，流作业的量走指标。这源于两类作业本身的差异，批有终值，流只有速率。
...
本方案定义 datasophon 平台中调度作业可视化的架构规范，范围覆盖调度元数据获取、任务级流量归属和工作流 DAG 的呈现。

上游是三个既有子系统，本方案不重建其中任何一个：

- **DolphinScheduler**（受管服务 `DS`，3.4.1）：工作流定义、实例、任务实例与 DAG 拓扑的唯一事实源。
- **Gravitino 血缘库**：作业与数据集的结构、运行记录，以及批作业的输出统计（行数、字节、文件数）。
...
- **调度面**的交接点是 DS Open API，承载结构与状态：有哪些工作流、这次跑到哪一步、每个任务是成功还是失败。
- **控制面**的交接点是 OpenLineage `RunEvent`，承载批作业的产出量：这次运行写了哪几张表、各多少行多少字节。
- **数据面**的交接点是 OTel 指标，承载流作业的运行期流量：此刻每秒多少行、自启动以来累计多少。
...
行数与容量依赖 OpenLineage 的 `OutputStatisticsOutputDatasetFacet`（含 `rowCount` / `size` / `fileCount`）。对照检索 OpenLineage 仓库**最新版本**：

| 统计类 facet | `integration/spark` | `integration/flink` |
|---|---:|---:|
| `OutputStatisticsOutputDatasetFacet` | **18 处** | **0 处** |
| `InputStatisticsInputDatasetFacet` | **15 处** | **0 处** |
| `DataQualityMetricsInputDatasetFacet` | 有 | **0 处** |

上述对照是在当前最新版本上复核的，所以这与版本新旧无关：Flink 集成对该 facet 从未有过实现，
不存在「升级即可获得」的路径。替代字段也没有。Flink 集成自有的 facet 只有 `CheckpointFacet`
（checkpoint 次数）和 `FlinkJobDetailsFacet`（仅作业 id），都不承载数据量。
社区两侧也没有在办提案：OpenLineage 侧无相关 issue/PR，Flink 上游的血缘工作全部指向拓扑方向
（connector 覆盖面、数据集命名），无一涉及统计量。

根因是挂载点的语义差异，不是实现进度：

| | 挂载点 | 事件语义 | 统计量是否随事件到达 |
|---|---|---|---|
| Spark | `SparkListenerTaskEnd` | 「数据搬完了」 | **是**，`outputMetrics` 就在参数里 |
| Flink | `JobStatusChangedListener`（FLIP-314） | 「作业状态变了」 | **否**，参数只有 `LineageGraph` 拓扑 |

Flink 的记录数计数器位于其 metric 子系统，与血缘监听器分属两条互不相通的管道。

> **口径**：写「Flink 拿不到权威行数，但拿得到观测行数」，不要写成笼统的「Flink 拿不到行数」。
> 流作业的累计量是可以做的，走指标（§2.3）；笼统的说法会被读成该能力不存在。
> 两者的差别在于权威值与重建值，见 §4.4 与 §9.4 R6。

所以「批任务显示总行数」这个需求在 Flink 批作业上开箱不成立，在 Spark 上成立且实测精确。
...
语义上累加是对的：`Delta` 加 `is_monotonic` 决定了正确算子是 `SUM` 而不是 `MAX`。这个判定不依赖对 reporter 实现的推断，两个字段随数据一并落库，可直接查询（另一条 reporter 路径导出的是累计值，必须用 `MAX`，见 §5.3 E2）。

精度则取决于采集链路是否完整，而且失败时没有任何提示。实测特征：

| 项 | 实测值 |
|---|---|
| 采样间隔 | **恒定 10 秒**（同一「作业 + 任务 + 子任务 + 算子」四元组下 min = avg = max，零抖动） |
| 正常期缺口 | **零**（连续数日无一处超过一个上报周期） |
| 异常期 | 曾观察到一次约 1.7 小时的采样丢失窗口：该窗口内**全部**上报分区间歇性丢样本，丢失约 **22%** |
| 该窗口的少算幅度 | 约 **23%**（按到达样本数反推；同期每样本行数保持稳定，说明吞吐未变、变的只是样本到达率） |
| 摊到全生命周期 | 约 **0.16%** |

误差集中在丢样窗口内，不会均匀摊薄。丢样本时查询正常返回，没有异常也没有日志，
拿到的是一个偏小但看起来完全正常的数字。血缘侧不是这样：丢事件的表现是「没有数据」，一眼能看出来。

> **架构含义**：累计量在语义上可重建，但它是观测重建值而不是权威值，
> 因此只能标注为约值，不可用于对账（§4.4 规则 3、§9.4 R6）。

## 2.4 七类静默错误

以下七类错误都不报错，只给出错误的数字或空白，因此必须在架构层面消除，测试层面拦不住：
...
以下两条不是本方案引入的，是在沙箱验证过程中撞出的既有缺陷。它们阻断本方案的一切落地：
DS 连工作流都建不出来时，调度可视化无从谈起。详见 `docs/ds-平台缺陷-任务插件缺失与S3凭据漂移-2026-08-25.md`。

| # | 缺陷 | 症状与影响 |
|---|---|---|
| P1 | **分发的 DS 包缺全部任务插件** | 安装包中仅有 `task-api` 与 `task-executor`，SPI 只注册 5 个 logic task。Shell / Spark / Flink / SQL 等任务在「创建工作流定义」阶段即被拒，而接口返回的是 `request parameter {0} is not valid`，真因 `Cannot find TaskChannel for : SHELL` 只在服务端日志里。平台装出来的 DS 目前不可用 |
| P2 | **DS 的对象存储凭据与实际不符** | 资源中心配置为 S3 形态但密钥不匹配。S3 客户端**只在服务启动时初始化一次**，因此错误被运行中的旧进程长期掩盖，**任一角色重启即无法启动**。修复须从服务元数据侧进行，否则平台重新下发配置会覆盖手工修复 |

> P2 的失败模式值得注意：服务一直在正常运行，只能说明它还没重启过，不能说明它的配置是对的。
...
| Spark 4.x / SPARK3 受管纳管 | 需换 Scala 2.13 构建线与 OpenLineage 版本（**下限 ≥1.37.0**，见 §9.3）、处理 HIVE 依赖，够独立立项 |
...
这是本方案的架构核心。调度对象与计算对象的绑定有四种做法，逐一评估：
...
选型看的是失败方式而非成功率。注入式标识在链路任一环断裂时，结果都是「这个节点没有指标」，用户看到 `—` 并有引导；名字匹配断裂时会给出看起来正确的错误数字，而这类看板的数字是要拿来做判断的。
...
两条路径的共同点：标识由调度侧生成，由计算侧原样携带，由查询侧精确匹配，中间不做任何推断。

> **标识必须带集群前缀（E7）**。DS 的内置变量给出的是裸数字，而血缘侧的绑定键是全局键、
> 没有命名空间，两套 DS 各有一个「任务实例 3」时会互相覆盖。因此标识格式定为
> `ds-<clusterId>-<taskInstanceId>`，由平台侧按同一规则拼装后查询。

> **流侧为何不复用批侧的 facet 注入**：流作业的量走指标而非血缘（§2.2），
> 指标的标签集由 reporter 决定，不接受任意注入，而 `job_name` 是标签集中唯一由作业自身完全控制的字段。
...
1. **状态一律以 DS 为准**，不以血缘或指标推断。血缘侧的「运行中」判定是 `terminal_state IS NULL AND updated_at >= now-24h`，没有心跳探活且走内存快照，与调度状态可能不一致。
2. **批任务的量是终值**，流作业的量是时序。两者在节点上的呈现形态不同，不可混用同一控件。
3. **累计量必须标注为约值**。它由 delta 求和得到，采集中断时会偏小，而且偏小时没有任何提示
   （§2.3 已量化：丢样窗口内少算约 23%）。约值与权威值不要用同一种视觉表达：
   批任务的行数来自写入方在事务结束时的报告，流作业的累计量来自观测侧重建，
   两者可信度不同，呈现形态一样就会被当成等价可比。

## 4.5 节点信息模型：一个任务对多个作业

「一个调度任务 = 一个计算作业」这个直觉假设不成立。Spark 官方集成的 `appendDatasetName` 默认开启，每个 action 或输出数据集各产生一条 OpenLineage job，实测一次 SQL 脚本提交产出了多条 job 与 run。
...
Spark 以 local 模式运行在 DS WorkerServer 所在节点，`spark-submit` 由 DS worker 本地拉起，所以 Spark 必须与 DS worker 同机。三台的 Spark 配置**必须字节级一致**，否则任务落到不同节点会出现「有时有行数、有时没有」的间歇性现象。

流侧同理。调度器提交 Flink 作业用的是本地客户端（`${FLINK_HOME}/bin/…`），
所以每个可能承接该任务的 worker 节点都必须装有 Flink 客户端。客户端只装在部分节点上时，
必须用调度器的 worker 分组把流任务钉到这些节点，否则任务随机落到无客户端的节点就会失败。
计算客户端与调度执行位置强耦合，这个约束在两种引擎上表现是一样的。
...
但不要沿用其「状态严格 == RUNNING」的过滤。既有实现曾因此在角色带 `EXISTS_ALARM`（进程实际健康）时把端点判为不可用，导致全平台查询失败，而 DS 的 ApiServer 长期带告警是常态。「哪些角色状态算可用」必须写成显式规则。

## 5.2 批指标通路

Spark 侧的 OpenLineage 接线已在平台元数据中完整存在（`SPARK3/service_ddl.json`）：jar 随框架包分发、listener 注册、transport 指向 Gravitino `/api/lineage`、api_key 鉴权、`capturedProperties` 声明。本方案对它的唯一增量是在 `capturedProperties` 中追加注入的调度标识 key。

Gravitino 侧的改造已完成并验收（原 G1/G2）。改造内容与设计要点：

- **解析做成配置项**，而不是再写死一个 key。新增 `storage.externalRunKeys`（逗号分隔、按序取首个非空、
  默认空则完全保持原行为），避免把「只认 `spark.app.id`」的错误换成「只认另一个固定 key」。
- **存储**：绑定键落在运行维度（`lineage_run.external_run_key`）而不是作业维度，并建索引。
  它是查询主键，而作业名字段无索引的教训就在同一张库里。
- **读取路径放宽**：不再限制为作业的当前运行，因此历史工作流实例能取到属于自己那一次的量。
- **查询接口按标识聚合该次运行的全部血缘作业**（下一段说明为何不是「取一个」）。

> **一次任务对应多个运行**。初版接口按「一个标识 → 一次运行」建模，实测把它否证了：
> 一次 SQL 会话产生 7 个 run（1 个应用级，其余按语句），统计只落在写入语句对应的那几个 run 上，
> 而应用级 run 的事件序号最大。于是「取最新的那个 run」恰好取到没有统计的应用级 run，
> 接口稳定返回空。改成按标识聚合全部 run、按输出数据集分列之后，行数与实际写入逐值一致。
...
> **三条 reporter 路径必须分清**（E2）：1.20 的 backport 与 2.x 原生是同一份代码，走 OTLP，导出 delta Sum，落 `otel_metrics_sum`；Flink 自带的 Prometheus reporter 把 Counter 误标为 gauge，落 `otel_metrics_gauge`，它也是三条里唯一提供累计值的路径。同时配置两个 reporter 会导致双重计数。

## 5.4 平台聚合层

累计量后台聚合是本方案唯一需要平台新增持久化状态的组件：

- 定时任务按周期查询上一周期的 delta，累加进平台累计表；
- 幂等键为 `job_id` + 周期时间戳，保证重启可恢复、重复执行不重复累加；
- 起点取 Flink REST 的 `start-time`（由引擎权威给出，不依赖调度侧时间戳精度）。

建议同时记录每周期的采样完整度。采样丢失是可检测的：上报间隔恒定（§2.3），
对同一「作业 + 任务 + 子任务 + 算子」四元组比较相邻样本的时间差，就能判定该周期是否完整。
把完整度和累计量一并落表，前端才能区分「约值可信」与「本周期缺数据」，
否则用户看到的永远是一个不带置信信息的数字。

> ⚠️ **四元组是唯一键，缺一维即失效**：同一子任务下存在多个算子，
> 分区键漏掉算子维度，不同算子的样本就会被混成同一序列，得出「间隔为 0、无缺口」的假象。

该组件的失败模式是重启丢累计、重复累加，以及采样丢失导致的静默少算。这与本方案其余部分完全不同，需单独设计测试。
...
未遵守约定的任务，节点显示为未绑定（`—` 加引导），不做任何降级推断。

集群前缀不可省略（E7）：DS 内置变量给的是裸数字，而绑定键在血缘侧是全局键。两侧使用同一套拼装规则，平台按此规则构造查询条件。

名字不能当标识，因为两侧的命名格式实测完全对不上。Spark 官方集成把 `job.name` 强制规范化为 `<规范化appName>.<规范化节点名>[.<数据集名>]`（小写、非字母数字转下划线、camelCase 拆词）；Flink 侧若不显式设置 `pipeline.name`，SQL 自动生成的名字形如 `insert-into_<catalog>.<库>.<表>_sink`，多 sink 时逗号拼接成数百字符。两者都不可能等于 DS 的任务名。

## 6.2 调度侧契约与其能力边界

| 能力 | 现状 | 架构影响 |
|---|---|---|
| 按项目列工作流定义 | 支持分页；**无 `releaseState` 过滤参数**；无分页的精简列表接口**不返回该字段** | 平台侧自造过滤只能二选一地承受「每页条数跳变」或「每次全量拉取」。**已裁决：按调度侧规则透传分页**，前端只标记上线状态、不做客户端筛选（原 G4 关闭） |
...
| 唯一键 | **`job_id` + `task_id` + `subtask_index` + `operator_name` 四元组**（§5.4） |
| 采样间隔 | 恒定 10 秒 |
| 点密度 | 实测 6.5k–7.2k 点 / 作业 / 小时 |

标签集里有 `job_name`，流侧才能只依赖 Doris、不必访问 Flink REST，这也消掉了「平台如何知道非受管 Flink 的地址」这个未解问题。

> **指标覆盖并不一致，没有一条通吃的查询**。实测同一环境下，算子级 `numRecordsIn` 覆盖全部作业，
> 而 sink 专用指标（`…operator.sink.numRecordsOut`）只覆盖其中一部分，取决于 connector 的实现形态。
> 因此取数必须保留「按算子名匹配 sink 算子」的回退路径，而这个匹配本身是 E2 类静默错误的高发点：
> 不同 connector 的 sink 算子命名形态不同，有的拆成写入和提交两个算子，有的融合为一个，
> 匹配不中时查询成功、结果为空、无异常无日志。
> 该匹配规则若在前后端各存一份拷贝，两处必须同步修改。
...
DS 的访问令牌以服务配置项形式存储。平台目前没有敏感字段机制，既有的数据库口令、签名密钥等同样是明文录入、明文存库、明文回显。本方案不打算为单个参数引入一套加解密体系，改用权限收敛：**为平台单独建立一个只读权限的 DS 用户，用它签发令牌**。

这样即使令牌泄露，影响面也限制在只读范围内，与本方案「只读展示」的定位一致。
...
| 结论 | 证据 |
|---|---|
| `app_link` 对 Flink/Spark 恒空，且为上游回归 | 上游 tag `3.4.1` 源码调用链复核（§2.1），并经接口实测：三条任务实例（含调度器原生 Spark 任务类型）该字段全部为 null |
| `outputStatistics` 仅 Spark 集成实现 | OpenLineage 仓库对照检索（§2.2） |
| **该缺失与 OpenLineage 版本无关，且无替代字段** | 对最新版本复核：Flink 集成对 `OutputStatistics` / `InputStatistics` / `DataQualityMetrics` 三类统计 facet 均 0 命中；其自有 facet 仅 `CheckpointFacet` 与 `FlinkJobDetailsFacet`，不承载数据量；社区两侧均无在办提案（§2.2） |
| **官方 Flink 集成用 `flink_job.jobId` 承载作业 id** | 与本平台上报器沿用的 `spark.app.id` 路径完全不同。若将来改用官方集成，血缘侧现有解析将确定性失效（见 R7） |
| **Delta 累加可重建累计量，但会静默少算** | 实测采样间隔恒定 10 秒、正常期零缺口；曾观察到一次约 1.7 小时的采样丢失窗口，丢 22% 样本，该窗口少算约 23%，摊到全生命周期约 0.16%。同期每样本行数稳定，证明吞吐未变（§2.3） |
| **sink 指标覆盖不一致** | 算子级 `numRecordsIn` 覆盖全部作业，sink 专用指标仅覆盖一部分，因此没有统一查询，须保留按算子名匹配的回退路径（§6.3） |
| **注入式标识可穿透到血缘事件** | 沙箱真实 Spark 作业：注入的 key 与 `spark.app.id` 并列出现在 `spark_properties` facet 中 |
| **调度侧变量替换在两种提交形态下均成立** | 内置变量在 Shell 脚本正文与原生 Spark 任务的附加参数中都被正确替换（任务日志实证） |
| **批链路端到端跑通，行数逐值精确** | 由调度器发起的两条工作流：写 700 / 234 行与 900 / 225 行，接口返回完全一致；两次均为「7 个 run、2 条输出」 |
| **流链路端到端跑通，标识可回指** | 由调度器发起的流作业：作业名回指到任务实例，Doris 按前缀命中后取出的 `job_id` 与引擎 REST 的作业 id 完全一致，速率非零 |
| **历史运行可独立取值** | 同一作业先后两次运行，按前一次的标识查询，返回的仍是前一次的量，不被后续运行覆盖 |
...
| **速率是近似值，口径必须写死** | 同一作业：数据源配置为 10 行/秒，按 `SUM(delta) / (最大时刻 − 最小时刻)` 算出约 22.8 行/秒，分母少算了首个采集周期。须标注为约值，与累计量口径一致 |
...
| OpenLineage(Spark) | 1.29.0 | 平台元数据钉定，实现 `outputStatistics`。已复核该版本覆盖 Spark 3.5.x，本方案无需变更。若将来上 Spark 4.x，下限是 1.37.0：1.34.0 才把依赖对齐到 Spark 4.0 正式版，1.37.0 才修复 `LogicalRelation` 构造器兼容，不修则 Spark 4 上多条写入路径直接抛异常 |
...
| R6 | **观测总量的精度受采集链路可用性支配** | 流作业累计量由 delta 求和重建，采样丢失会静默少算，且与「数据本来就少」无法区分。实测正常期采样间隔恒定、零缺口，但出现过丢失窗口，该窗口少算约 23%。因此累计量只能标注为约值，不可用于对账；建议同时暴露采样完整度（§5.4）。此风险的影响面不限于本方案，同一采集链路还承载着平台其它指标消费方 |
| R7 | **上报器与血缘侧解析强耦合** | 血缘侧的作业 id 解析是照本平台上报器的字段约定写的，而官方 Flink 集成用的是另一个字段路径（§9.1）。更换上报器会打断解析，这是确定会发生的影响，不是可能性 |
