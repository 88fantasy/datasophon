...
import { datasophonMcpServer } from "./tools";
...
function sendChunk(send: SendFn, content: string): void {
...
function sendToolEvent(send: SendFn, info: ToolCallInfo): void {
  const payload = JSON.stringify(info);
  const b64 = Buffer.from(payload).toString("base64");
  sendChunk(send, `<tool-call>${b64}</tool-call>`);
}
    ...
    for await (const message of query({
      ...
      if (message.type === "stream_event") {
        ...
        if (
          ...
          sendChunk(send, ev.delta.text as string);
        }
