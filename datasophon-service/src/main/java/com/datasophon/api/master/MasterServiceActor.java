 ...
 */
@Slf4j
public class MasterServiceActor extends TypedActor<ExecuteServiceRoleCommand> {

    @Override
    protected void doOnReceive(ExecuteServiceRoleCommand srvRoleCmd) throws Throwable {
        ...
        for (ServiceRoleInfo serviceRoleInfo : masterRoles) {
            log.info("{} service role size is {}", serviceRoleInfo.getName(), masterRoles.size());
            if (CancelCommandMap.exists(serviceRoleInfo.getHostCommandId())) {
                log.warn("cmd {} {} in host {} canceled", srvRoleCmd.getCommandType().getCommandName(Constants.CN), serviceRoleInfo.getName(), serviceRoleInfo.getHostname());
                continue;
            }
            ...
            log.info("{} enable ranger plugin is {}", serviceRoleInfo.getParentName(), enableRangerPlugin);

            log.info("{} {} in host {}, begin to generate config map", srvRoleCmd.getCommandType().getCommandName(Constants.CN), serviceRoleInfo.getName(), serviceRoleInfo.getHostname());
            ...
            log.info("{} {} in host {}", srvRoleCmd.getCommandType().getCommandName(Constants.CN), serviceRoleInfo.getName(), serviceRoleInfo.getHostname());
...
//            FIXME 按照旧代码的逻辑，execResult可能造成NPE。
  ...
  private ExecResult doInstallService(ServiceRoleInfo serviceRoleInfo, ExecuteServiceRoleCommand executeServiceRoleCommand, ExecContext ctx) {
    ExecResult execResult = null;
    try {
      log.info("start to install {} in host {}", serviceRoleInfo.getName(), serviceRoleInfo.getHostname());
      ...
      if (Objects.nonNull(execResult) && execResult.getExecResult()) {
        ...
        if (ServiceRoleType.MASTER.equals(serviceRoleInfo.getRoleType()) && ctx.isAllSuccess()) {
          log.info("all master role has installed");
          ProcessUtils.tellCommandActorResult(serviceRoleInfo.getParentName(), executeServiceRoleCommand, ServiceExecuteState.SUCCESS);
        }
        log.info("{} install success in {}", serviceRoleInfo.getName(), serviceRoleInfo.getHostname());
      } else {
        if (ServiceRoleType.MASTER.equals(serviceRoleInfo.getRoleType())) {
          log.info("{} install failed in {}", serviceRoleInfo.getName(), serviceRoleInfo.getHostname());
      ...
      log.info("{} install failed in {}", serviceRoleInfo.getName(), serviceRoleInfo.getHostname());
      ProcessUtils.tellCommandActorResult(serviceRoleInfo.getParentName(), executeServiceRoleCommand, ServiceExecuteState.ERROR);
      log.error(ProcessUtils.getExceptionMessage(e));
    }
  ...
  private ExecResult doStartService(ServiceRoleInfo serviceRoleInfo, ExecuteServiceRoleCommand executeServiceRoleCommand, ExecContext ctx, boolean needReConfig) {
    ExecResult execResult = null;
    try {
      log.info("start  {} in host {}", serviceRoleInfo.getName(), serviceRoleInfo.getHostname());
      execResult = ProcessUtils.startService(serviceRoleInfo, needReConfig);
      if (Objects.nonNull(execResult) && execResult.getExecResult()) {
        ctx.successNumInc();
        if (ServiceRoleType.MASTER.equals(serviceRoleInfo.getRoleType()) && ctx.isAllSuccess()) {
          log.info("{} start success", serviceRoleInfo.getParentName());
          ProcessUtils.tellCommandActorResult(serviceRoleInfo.getParentName(), executeServiceRoleCommand, ServiceExecuteState.SUCCESS);
        }
        ...
        if (ServiceRoleType.MASTER.equals(serviceRoleInfo.getRoleType())) {
          log.info("{} start failed", serviceRoleInfo.getParentName());
          ProcessUtils.tellCommandActorResult(serviceRoleInfo.getParentName(), executeServiceRoleCommand, ServiceExecuteState.ERROR);
        }
      }
    } catch (Exception e) {
  ...
  private ExecResult doStopService(ServiceRoleInfo serviceRoleInfo, ExecuteServiceRoleCommand executeServiceRoleCommand, ExecContext ctx) {
    ExecResult execResult = null;
    try {
      log.info("stop {} in host {}", serviceRoleInfo.getName(), serviceRoleInfo.getHostname());
      ...
      if (Objects.nonNull(execResult) && execResult.getExecResult()) { // 执行成功
        ctx.successNumInc();
        if (ServiceRoleType.MASTER.equals(serviceRoleInfo.getRoleType()) && ctx.isAllSuccess()) {
          log.info("{} stop success", serviceRoleInfo.getParentName());
          ProcessUtils.tellCommandActorResult(serviceRoleInfo.getParentName(), executeServiceRoleCommand, ServiceExecuteState.SUCCESS);
        }
        ...
        if (ServiceRoleType.MASTER.equals(serviceRoleInfo.getRoleType())) {
          log.info("{} stop failed", serviceRoleInfo.getParentName());
          ProcessUtils.tellCommandActorResult(serviceRoleInfo.getParentName(), executeServiceRoleCommand, ServiceExecuteState.ERROR);
        }
      }
    } catch (Exception e) {
  ...
  private ExecResult doRestartService(ServiceRoleInfo serviceRoleInfo, ExecuteServiceRoleCommand executeServiceRoleCommand, ExecContext ctx, boolean needReConfig) {
    ExecResult execResult = null;
    try {
      log.info("restart {} in host {}", serviceRoleInfo.getName(), serviceRoleInfo.getHostname());

      execResult = ProcessUtils.restartService(serviceRoleInfo, needReConfig);
      if (Objects.nonNull(execResult) && execResult.getExecResult()) {
        ctx.successNumInc();
        if (ServiceRoleType.MASTER.equals(serviceRoleInfo.getRoleType()) && ctx.isAllSuccess()) {
          log.info("{} restart success", serviceRoleInfo.getParentName());
          ProcessUtils.tellCommandActorResult(serviceRoleInfo.getParentName(), executeServiceRoleCommand, ServiceExecuteState.SUCCESS);
        }
        // update service role state is running
        ...
        if (ServiceRoleType.MASTER.equals(serviceRoleInfo.getRoleType())) {
          log.info("{} restart failed", serviceRoleInfo.getParentName());
          ProcessUtils.tellCommandActorResult(serviceRoleInfo.getParentName(), executeServiceRoleCommand, ServiceExecuteState.ERROR);
        }
      }
    } catch (Exception e) {
  ...
  private ExecResult doServiceAction(ServiceRoleInfo srvInfo, ExecuteServiceRoleCommand cmd, ExecContext ctx, Callable<ExecResult> callable) {
    try {
      log.info("{} {} in host {}", cmd.getCommandType().getCommandName(Constants.CN), srvInfo.getName(), srvInfo.getHostname());
      ExecResult execResult = callable.call();
      if (execResult.isSuccess()) {
        ctx.successNumInc();
        if (ServiceRoleType.MASTER.equals(srvInfo.getRoleType()) && ctx.isAllSuccess()) {
          log.info("{} {} success", srvInfo.getParentName(), cmd.getCommandType().getCommandName(Constants.CN));
        ...
        if (ServiceRoleType.MASTER.equals(srvInfo.getRoleType())) {
          log.info("{} {} failed", srvInfo.getParentName(), cmd.getCommandType().getCommandName(Constants.CN));
