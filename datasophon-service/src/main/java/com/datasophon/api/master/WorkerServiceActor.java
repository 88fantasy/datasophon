...
public class WorkerServiceActor extends TypedActor<ExecuteServiceRoleCommand> {

    private static final Logger logger = LoggerFactory.getLogger(WorkerServiceActor.class);

    @Override
    protected void doOnReceive(ExecuteServiceRoleCommand executeServiceRoleCommand) throws Throwable {
        ...
        switch (executeServiceRoleCommand.getCommandType()) {
            ...
            case RESTART_SERVICE:
                ...
                break;
            default:
                logger.warn("WorkerServiceActor receive an unknown CommandType {}, it's that true?", executeServiceRoleCommand.getCommandType());
                execResult = ExecResult.error(String.format("WorkerServiceActor receive an unknown CommandType %s",
                        executeServiceRoleCommand.getCommandType() == null ? "null" : executeServiceRoleCommand.getCommandType().getValue()));
                break;
        }
