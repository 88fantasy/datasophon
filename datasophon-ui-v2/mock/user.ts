...
export default {
  // 支持值为 Object 和 Array
  'GET /ddh/api/v2/currentUser': (_req: Request, res: Response) => {
    if (!getAccess()) {
      res.status(401).send({
        data: {
          isLogin: false,
  ...
  'POST /ddh/api/v2/login/account': async (req: Request, res: Response) => {
  ...
  'POST /ddh/api/v2/logout': (_req: Request, res: Response) => {
    access = '';
