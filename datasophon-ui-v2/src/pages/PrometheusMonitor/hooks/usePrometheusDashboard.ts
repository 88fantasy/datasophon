...
import type { TimeSeriesPoint } from '../mock/prometheusMockData';
...
import type { DashboardVariables } from '../utils/promql';
import type { InstantPanelDef } from '../panelQueries';
import {
...
const EMPTY_SERIES: Record<string, TimeSeriesPoint[]> = Object.fromEntries(
  ['P08', 'P09', 'P10', 'P11', 'P12', 'P13', 'P14', 'P15', 'P16', 'P17',
   'P18', 'P19', 'P20', 'P21', 'P22', 'P23', 'P24'].map((k) => [k, []]),
);
    ...
    async function fetchAll() {
      ...
      try {
        ...
        async function multiRange(panelId: string) {
          ...
          const parts = await Promise.all(
            def.queries.map(async ({ label, promql }) => {
              ...
              return { label, matrix: res?.data ?? { resultType: 'matrix' as const, result: [] } };
            }),
          );
        ...
        const [
          ...
          queryInstant({ query: replaceVars((PANEL_QUERIES.P07 as InstantPanelDef).promql, vars), time: end, clusterId })
            .then((r) => r?.data ?? { resultType: 'vector' as const, result: [] }),
          // up 查询（派生 instances/jobs）
          queryInstant({ query: 'up', time: end, clusterId })
            .then((r) => r?.data ?? { resultType: 'vector' as const, result: [] }),
