...
describe('PrometheusMonitor promql helpers', () => {
  ...
  describe('matrixToSeries', () => {
    it('converts matrix result to TimeSeriesPoint[] using specified seriesKey', () => {
      ...
      expect(points[0]).toEqual({ time: 1718000000000, value: 1.5, series: 'localhost:9090' });
      expect(points[1]).toEqual({ time: 1718000030000, value: 2.0, series: 'localhost:9090' });
    });
  ...
  describe('mergeNamedSeries', () => {
    it('merges multiple labeled matrices into single series array', () => {
      const m1 = {
        resultType: 'matrix' as const,
        result: [{ metric: {}, values: [[1718000000, '1'] as [number, string]] }],
      };
      const m2 = {
        resultType: 'matrix' as const,
        result: [{ metric: {}, values: [[1718000000, '2'] as [number, string]] }],
      };
  ...
  describe('vectorToScalar', () => {
    it('returns numeric value from first result', () => {
      const v = {
        ...
        result: [{ metric: {}, value: [1718000000, '99.8'] as [number, string] }],
      };
  ...
  describe('deriveInstancesAndJobs', () => {
    it('extracts distinct instances and jobs from up vector', () => {
      const v = {
        resultType: 'vector' as const,
        result: [
          { metric: { instance: 'h1:9090', job: 'prometheus' }, value: [1718000000, '1'] as [number, string] },
          { metric: { instance: 'h2:9090', job: 'node' }, value: [1718000000, '1'] as [number, string] },
          { metric: { instance: 'h1:9090', job: 'node' }, value: [1718000000, '1'] as [number, string] },
        ],
      };
    ...
    it('returns empty arrays for empty vector', () => {
      const { instances, jobs } = deriveInstancesAndJobs({ resultType: 'vector', result: [] });
