...
describe('isReadonlyOtelBuiltinRule', () => {
  it('locks Nexus and Doris OTel built-in alert rules', () => {
    expect(isReadonlyOtelBuiltinRule({ alertQuotaName: 'Nexus实例只读' })).toBe(
      true,
    );
    expect(
      isReadonlyOtelBuiltinRule({ alertQuotaName: 'Doris查询错误率' }),
    ).toBe(true);
  });

  it('does not lock ordinary alert rules or new records', () => {
    expect(isReadonlyOtelBuiltinRule({ alertQuotaName: '主机CPU使用率' })).toBe(
      false,
    );
