    ...
    public ExecResult handler(ServiceRoleOperateCommand command) {
        ...
        if (command.getCommandType().equals(CommandType.INSTALL_SERVICE)) {
            ...
            if (envFile.exists()) {
                ...
                Optional<String> optionalUrl = lines.stream().filter(line -> line.contains("SPRING_DATASOURCE_URL")).findFirst();
                Optional<String> optionaUsername = lines.stream().filter(line -> line.contains("SPRING_DATASOURCE_USERNAME")).findFirst();
                Optional<String> optionalPassword = lines.stream().filter(line -> line.contains("SPRING_DATASOURCE_PASSWORD")).findFirst();

                if (optionalUrl.isPresent() && optionaUsername.isPresent() && optionalPassword.isPresent()) {
                    Connection con = null;
                    try {
                        con = DbUtil.use(new SimpleDataSource(getValue(optionalUrl.get()), getValue(optionaUsername.get()), getValue(optionalPassword.get()))).getConnection();
                        List<Entity> entityList = SqlExecutor.query(con, "SHOW TABLES LIKE 't_ds_version'", new EntityListHandler());
