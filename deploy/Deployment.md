...
镜像启动成功后，在浏览器中访问 <http://docker_ip:8080/ddh> 进入登录页。 默认用户名和密码为admin/admin123
...
```shell
docker run -d --name datasophon -v your_path/datasophon.conf:/datasophon/conf/datasophon.conf -p 8080:8080 datasophon/datasophon
```
...
```shell
docker run -d --name datasophon -v your_path/datasophon.conf:/datasophon/conf/datasophon.conf -v your_path/DDP:/data/install_datasophon/packages -p 8080:8080 datasophon/datasophon
```
