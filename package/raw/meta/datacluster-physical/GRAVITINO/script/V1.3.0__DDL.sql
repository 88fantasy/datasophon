...
-- 来源：apache/gravitino v1.3.0 scripts/mysql/schema-1.3.0-mysql.sql，逐字复制仅重命名。
-- 供 datasophon initDb hook 使用（Migration.getMigrations 按 V<version>__DDL.sql 正则匹配）。
-- 升级 Gravitino 版本时，同步从对应 tag 的 scripts/mysql/schema-<version>-mysql.sql 取增量脚本，
-- 命名为 V<version>__DDL.sql 放入本目录，不要改用官方 upgrade-*-mysql.sql（命名格式不兼容本 hook）。
