...
usage="Usage: control.sh (start|stop|restart|status) <command>"
...
start_exporter(){
  ...
  echo starting $command, logging to $log
  exec_command="$VALKEY_DIR/redis-exporter/redis_exporter -redis.addr 127.0.0.1:$port -web.listen-address 0.0.0.0:${valkeyExporterPort} -redis.password $pwd -include-config-metrics true -include-system-metrics true"
  ...
  (start)
    case $command in
        ...
        (exporter)
          ...
          ;;
        (*)
          echo "Invalid second parameter. Usage: $0 start {master|exporter}"
          exit 1
          ;;
    esac
    ;;
