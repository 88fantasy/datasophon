...
本页列出三套执行序列的完整步骤。数据来源：`internal/plan/registry.go`（initALL / initSingleNode）和 `internal/cli/create/initializer.go`（standalone）。
...
| 4 | `init-rustfs` | 安装 RustFS | RustFS 节点 | `registry.enable && rustfs.enable` | [rustfs](../commands/init/repo/rustfs.md) |
...
| 14 | `init-offline-server` | yum/apt 离线源服务 | 离线源节点 | `yumServer.enable` | [offlineServer](../commands/init/repo/offlineserver.md) |
...
| 21 | `init-nmap` | 安装 nmap | nmap 节点 | `nmapServer.enable` | [nmap](../commands/init/network/nmap.md) |
| 22 | `init-ntp-server` | 配置 NTP Server | NTP Server 节点 | `ntpServer.enable` | [ntpserver](../commands/init/network/ntpserver.md) |
| 23 | `init-ntp-slave` | 配置 NTP Slave | 全节点 | `ntpServer.enable` | [ntpslave](../commands/init/network/ntpslave.md) |
| 24 | `init-mysql` | 安装 MySQL | MySQL 节点 | `mysql.enable` | [mysql](../commands/init/db/mysql.md) |
...
<a name="initsinglenode-17-步"></a>

## initSingleNode — 17 步（新增节点初始化）

由 `create node`（批量模式，读取 `addNodes` 列表）触发，通过 plan 引擎持久化，支持断点续跑。

| 序号 | Step ID | 步骤名 | 节点范围 | 触发条件 | 对应命令页 |
|---|---|---|---|---|---|
| 1 | `init-bin-package` | 分发资源包 | addNodes | 无 | [bin_packages](../commands/init/packages/bin_packages.md) |
| 2 | `init-bash` | shell bash 设置 | addNodes | 无 | [bash](../commands/init/system/bash.md) |
| 3 | `init-tar` | 安装 tar | addNodes | 无 | [tar](../commands/init/packages/tar.md) |
| 4 | `init-jdk8` | 安装 JDK 8 | addNodes | 无 | [jdk8](../commands/init/packages/jdk8.md) |
| 5 | `init-jdk17` | 安装 JDK 17 | addNodes | 无 | [jdk17](../commands/init/packages/jdk17.md) |
| 6 | `init-osuser` | 创建 hadoop 用户和组 | addNodes | 无 | [osUser](../commands/init/system/osuser.md) |
| 7 | `init-firewall` | 关闭防火墙 | addNodes | 无 | [firewall](../commands/init/system/firewall.md) |
| 8 | `init-selinux` | 关闭 SELinux | addNodes | 无 | [selinux](../commands/init/system/selinux.md) |
| 9 | `init-swap` | 关闭 Swap | addNodes | 无 | [swap](../commands/init/system/swap.md) |
| 10 | `init-offline-nodes` | yum/apt 离线源节点配置 | addNodes | `yumServer.enable \|\| registry.enable` | [offlineSlave](../commands/init/repo/offlineslave.md) |
| 11 | `init-library` | 初始化依赖库 | addNodes | 无 | [library](../commands/init/system/library.md) |
| 12 | `init-os-safe-conf` | 安全配置 | addNodes | 无 | [osSafeConf](../commands/init/system/ossafeconf.md) |
| 13 | `init-system-conf` | 优化系统配置 | addNodes | 无 | [system-conf](../commands/init/system/system-conf.md) |
| 14 | `init-hostname` | 配置 hostname | addNodes | 无 | [hostname](../commands/init/network/hostname.md) |
| 15 | `init-all-host` | 配置 /etc/hosts（全节点） | addNodes + nodes | 无 | [allHost](../commands/init/network/allhost.md) |
| 16 | `init-ntp-slave` | 配置 NTP Slave | addNodes | `ntpServer.enable` | [ntpslave](../commands/init/network/ntpslave.md) |
| 17 | `init-hugepage` | 关闭透明大页 | addNodes | 无 | [hugePage](../commands/init/system/hugepage.md) |

> 第 15 步 `init-all-host` 使用 `buildAllHostBoth`，会同时更新 addNodes 和已有 nodes 的 /etc/hosts，确保新老节点互相可达。

## standalone — 10 步（独立节点初始化）

由 `create node --ip <IP>` 触发，**不使用 plan 引擎**，硬编码顺序执行，不支持断点续跑。
