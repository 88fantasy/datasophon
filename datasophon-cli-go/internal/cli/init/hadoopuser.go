...
type InitHadoopUser struct{ TaskBase }

func (t *InitHadoopUser) Name() string { return "初始化用户" }

func (t *InitHadoopUser) Handle(client *ssh.Client, dryRun bool) error {
	return t.doRun(executor.NewSSHExecutor(client, dryRun))
}

func (t *InitHadoopUser) doRun(exec executor.Executor) error {
	// 创建 hadoop 组
