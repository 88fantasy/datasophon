...
// InitHelm 对应 Java InitHelm — 安装 Helm（仅当 enableKubernetesCluster=true）。
type InitHelm struct {
	TaskBase
...
func (t *InitHelm) Name() string { return "安装helm" }

func (t *InitHelm) Handle(client *ssh.Client, dryRun bool) error {
	return t.doRun(executor.NewSSHExecutor(client, dryRun))
}

func (t *InitHelm) Command(dryRun *bool) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "helm",
		Short: "安装 Helm",
...
func (t *InitHelm) doRun(exec executor.Executor) error {
	if !t.EnableK8sCluster {
		slog.Info("k8s 集群安装未开启，跳过 helm 安装")
		return nil
	}
	if r := exec.ExecShell("helm version"); r.Success {
		slog.Info("helm 已安装")
		return nil
	}
	...
	softPath := fmt.Sprintf("%s/helm", t.InstallPath)
