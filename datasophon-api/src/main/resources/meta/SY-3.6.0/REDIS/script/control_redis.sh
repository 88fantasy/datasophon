...
start_exporter(){
  ...
  echo starting $command, logging to $log
  exec_command="$REDIS_DIR/redis-exporter/redis_exporter -redis.addr $HOSTNAME:$port -web.listen-address 0.0.0.0:9121 -redis.password $pwd -include-config-metrics true -include-system-metrics true"
