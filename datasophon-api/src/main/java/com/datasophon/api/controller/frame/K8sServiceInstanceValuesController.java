...
public class K8sServiceInstanceValuesController extends ApiController {
    ...
    public Result getValueFromRepo(@PathVariable Integer serviceId, @Schema(description = "部署类型，可选值，helm, k8s") String artifactType) {
        String result = k8sServiceInstanceValuesService.getValueFromRepo(serviceId, artifactType);
        return Result.success(result);
    }

    @PostMapping("save")
    @Operation(summary = "保存服务实例 values")
    @ApiResponse(content = {
            @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = K8sServiceInstanceValues.class))
    })
    public Result save(@Validated @RequestBody K8sServiceInstanceValuesSaveDTO values) {
        K8sServiceInstanceValues result = k8sServiceInstanceValuesService.save(values);
        return Result.success(result);
    }
