...
export default {
  ...
  startExecuteCommand:
    path + "/api/cluster/service/command/startExecuteCommand", // 启动执行指令
  generateCommand: path + "/api/cluster/service/command/generateCommand", // 生成服务操作指令
  getServiceCommandlist:
    path + "/api/cluster/service/command/getServiceCommandlist", // 查询服务安装指令列表1
  getServiceHostList: path + "/api/cluster/service/command/host/list", // 查询服务安装对应主机列表
  getServiceRoleOrderList:
    path + "/api/cluster/service/command/host/command/list", // 查询主机上服务角色指令列表3
  getLog: path + "/cluster/service/role/instance/getLog", // 服务实例-查看日志
  getHostCommandLog:
    path + "/api/cluster/service/command/host/command/getHostCommandLog", // 查询主机上服务角色指令3日志
